...
def chat(prompt):
    ...
    if not os.path.exists("GeminiAI"):
        os.mkdir("GeminiAI")
    with open(f"GeminiAI/{''.join(prompt.split('about')[1:]).strip()}.txt", "w") as f:
        f.write(text3)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if "play music" in query.lower():
            ...
            # for pid in (process.pid for process in psutil.process_iter() if process.name() == "C:/Users/Rajukumar/OneDrive/Desktop/Media Player.lnk"):
            #     os.kill(pid)
            os.kill("C:/Users/Rajukumar/OneDrive/Desktop/Media Player.lnk")
            speaker.say("Opening Music boss")
            speaker.runAndWait()
        ...
        if "Friday".lower() in query.lower():
        ...
        elif "who are you" in query or "what can you do".lower() in query.lower() or 'hu r u'.lower() in query.lower():
